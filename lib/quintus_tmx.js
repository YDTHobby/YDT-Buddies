...
// Add this to your quintus_tmx.js file, replacing the existing Q._tmxProcessObjectLayer function

// Add this to your quintus_tmx.js file, replacing the existing Q._tmxProcessObjectLayer function

Q._tmxProcessObjectLayer = function(stage,gidMap,tileProperties,layer) {
   console.log("Processing object layer:", layer);
   ...
   for(var i=0;i < objects.length;i++) {
     var obj = objects[i],
         ...
         x = attr(obj,'x'),
         y = attr(obj,'y'),
         width = attr(obj,'width'),
         height = attr(obj,'height'),
         name = attr(obj, 'name'),
         type = attr(obj, 'type'),
         properties = {},
         overrideProperties = parseProperties(obj);
         
     console.log("Processing object:", {name: name, type: type, gid: gid, x: x, y: y, props: overrideProperties});

     // Handle tile-based objects (original functionality)
     if(gid) {
     ...
     // Handle direct class-based objects (new functionality)
     else if(overrideProperties['Class'] || type) {
       // Use either the Class property or the type attribute as the class name
       var className = overrideProperties['Class'] || type;
       properties = { Class: className };
     } else {
       console.warn("TMX Object has neither gid nor Class/type, skipping:", obj);
       continue;
     }

     var className = properties['Class'];
     if(!className) { throw "Invalid TMX Object Class: " + className; }

     // Check if the class exists
     if(!Q[className]) {
       console.warn("TMX Object Class not found: " + className + ", skipping object");
       continue;
     }
     ...
     var p = Q._extend({
       ...
       y: y,
       name: name
     }, Q._extend(properties, overrideProperties));
     ...
     // For tile-based objects, adjust position to center
     if(gid) {
