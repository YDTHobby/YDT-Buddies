...
Q._tmxProcessObjectLayer = function(stage, gidMap, tileProperties, layer) {
  ...
  for (var i = 0; i < objects.length; i++) {
    ...
    if (gid) {
      ...
      className = classAlias[type] || type;
    }

    if (!className || !Q[className]) {
      console.warn("Skipping object: missing or invalid Class for object", name || type);
      continue;
    }
    ...
    stage.insert(new Q[className](p));
  }
};
